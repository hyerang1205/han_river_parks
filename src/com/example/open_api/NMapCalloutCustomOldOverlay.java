...
public class NMapCalloutCustomOldOverlay extends NMapCalloutOverlay {
	...
	private static final float CALLOUT_BACKGROUND_HEIGHT = CALLOUT_PADDING_Y
			+ CALLOUT_TEXT_SIZE + CALLOUT_PADDING_Y;
	...
	public NMapCalloutCustomOldOverlay(NMapOverlay itemOverlay,
		...
		// set font size
		mTextPaint.setTextSize(CALLOUT_TEXT_SIZE
				* NMapResourceProvider.getScaleFactor());
		// set font type
		...
		mPaddingOffset = NMapResourceProvider
				.toPixelFromDIP(CALLOUT_PADDING_OFFSET);
		mPaddingY = NMapResourceProvider.toPixelFromDIP(CALLOUT_PADDING_Y);
		mMinimumWidth = NMapResourceProvider
				.toPixelFromDIP(CALLOUT_MIMIMUM_WIDTH);
		mTotalHeight = NMapResourceProvider
				.toPixelFromDIP(CALLOUT_TOTAL_HEIGHT);
		mBackgroundHeight = NMapResourceProvider
				.toPixelFromDIP(CALLOUT_BACKGROUND_HEIGHT);
		...
		mTitleOffsetY = NMapResourceProvider
				.toPixelFromDIP(CALLOUT_TITLE_OFFSET_Y);
		...
		if (mDrawableRightButton != null) {
			if (hasRightAccessory) {
				mCalloutRightButtonWidth = mDrawableRightButton[0]
						.getIntrinsicWidth();
				mCalloutRightButtonHeight = mDrawableRightButton[0]
						.getIntrinsicHeight();
			} else {
				mCalloutRightButtonWidth = NMapResourceProvider
						.toPixelFromDIP(CALLOUT_RIGHT_BUTTON_WIDTH);
				mCalloutRightButtonHeight = NMapResourceProvider
						.toPixelFromDIP(CALLOUT_RIGHT_BUTTON_HEIGHT);
			}
	...
	protected void drawCallout(Canvas canvas, NMapView mapView, boolean shadow,
		...
		mOffsetY = mTempRectF.top + mPaddingY + mTextPaint.getTextSize()
				+ mTitleOffsetY;
		...
		if (mDrawableRightButton != null) {
			...
			top = mTempRectF.top
					+ (mBackgroundHeight - mCalloutRightButtonHeight) / 2;
			...
			if (mRightButtonText != null) {
				mTextPaint.getTextBounds(mRightButtonText, 0,
						mRightButtonText.length(), mTempRect);

				left = mRightButtonRect.left
						+ (mCalloutRightButtonWidth - mTempRect.width()) / 2;
				top = mRightButtonRect.top
						+ (mCalloutRightButtonHeight - mTempRect.height()) / 2
						+ mTempRect.height() + mTitleOffsetY;
	...
	private void adjustTextBounds(NMapView mapView) {
		...
		int mapViewWidth = mapView.getMapController()
				.getViewFrameVisibleWidth();
		if (mTitleTruncated == null || mWidthTitleTruncated != mapViewWidth) {
			mWidthTitleTruncated = mapViewWidth;
			float maxWidth = mWidthTitleTruncated - 2 * mMarginX - 2
					* mPaddingX;
			if (DEBUG) {
				Log.i(LOG_TAG, "adjustTextBounds: maxWidth=" + maxWidth
						+ ", mMarginX=" + mMarginX + ", mPaddingX=" + mPaddingX);
			}
			...
			if (mTailText != null) {
				mTextPaint.getTextBounds(mTailText, 0, mTailText.length(),
						mTempRect);
			...
			final String title = TextUtils.ellipsize(mOverlayItem.getTitle(),
					mTextPaint, maxWidth, TextUtils.TruncateAt.END).toString();

			mTitleTruncated = title;

			if (DEBUG) {
				Log.i(LOG_TAG,
						"adjustTextBounds: mTitleTruncated=" + mTitleTruncated
								+ ", length=" + mTitleTruncated.length());
			}
		}

		mTextPaint.getTextBounds(mTitleTruncated, 0, mTitleTruncated.length(),
				mTempRect);
		...
		if (DEBUG) {
			Log.i(LOG_TAG,
					"adjustTextBounds: mTempRect.width=" + mTempRect.width()
							+ ", mTempRect.height=" + mTempRect.height());
		}
		...
		// set position
		float left = mTempPoint.x
				- (int) (mTempRectF.width() * mOverlayItem.getAnchorXRatio());
		float top = mTempPoint.y
				- (int) (mItemBounds.height() * mOverlayItem.getAnchorYRatio())
				- mItemGapY - mTotalHeight;
		mTempRectF.set(left, top, left + mTempRectF.width(),
				top + mTempRectF.height());
